...
def auto_tune_prices(base_prices, prod_data, retail_price_map):
    ...
    for iteration in range(200):
        ...
        max_adj = 0.0
        for pname in prod_data:
            if pname in base_prices:  # 锚点不变
                continue
            cur = profits.get(pname, 0)
            if cur <= 0:
                # 利润为0或负，提价20%
                prices[pname] = round(prices[pname] * 1.2, 2)
                max_adj = max(max_adj, 0.2)
            else:
                ratio = cur / target
                if ratio > 1.15:
                    new_price = round(prices[pname] * (1.0 / ratio), 2)
                elif ratio < 0.85:
                    new_price = round(prices[pname] * (1.0 + (1.0 - ratio)), 2)
                else:
                    continue
                ...
                if new_price > 0:
                    ...
                    max_adj = max(max_adj, abs(new_price - prices.get(pname, 0)) / max(prices.get(pname, 1), 0.01))
...
try:
    ...
    print("正在为新产品自动生成均衡价...")
    full_base = auto_tune_prices(BASE_GUIDE_PRICES, prod_data, retail_price_map)
    with open("static_base_prices.json", "w") as f:
        json.dump(full_base, f, indent=2)
    print("已生成 static_base_prices.json")
