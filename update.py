...
while remaining:
    ...
    for p in remaining:
        ...
        if all_known:
            ...
            price = round((mat + labor) * (1 + PROFIT_RATE), 2)
            # 零售品：基石价直接取 API 基础价（可保证零售端有利润）
...
for _ in range(3):
    for pname in prod_data:
        ...
        for ing, amount in recipe.items():
            per = amount / batch
            mat_cost += per * prices.get(ing, 0)
        min_price = mat_cost * 1.02  # 原料成本 + 2%
...
print(f"✅ 统一倍率 {unified_mult:.4f}，指导价已生成（成本加成15%）")
